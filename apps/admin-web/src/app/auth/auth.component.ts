...
export class AuthComponent {
  ...
  protected submit(): void {
    ...
    request.subscribe({
      next: ({ access_token }) => { session.accessToken = access_token; this.router.navigateByUrl('/workspaces'); },
      error: () => this.error.set('Could not sign in. Check your credentials and try again.'),
    });
  }
}
